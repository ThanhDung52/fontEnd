...
import { Box, Button, Divider, Select, MenuItem, FormControl, InputLabel, TextField, CircularProgress } from "@mui/material";
import { ErrorMessage, Field, Form, Formik } from "formik";
import { useDispatch, useSelector } from "react-redux";
import { createOrder } from "../State/Order/Action";
import * as Yup from "yup";
import { removeCartItem } from "../State/Cart/Action";
import { getProvinces, getDistricts, getWards } from "../State/GHN_API/GhnService";
...
const Cart = () => {
    ...
    // Tính phí dịch vụ
    const shippingData = {
        // Cấu hình dữ liệu theo yêu cầu của GHN API
        from_district_id: 1442, // ID của quận nơi hàng được gửi đi
        to_district_id: values.district, // ID quận đến
        weight: 1000, // Khối lượng sản phẩm (gram)
        length: 20, // Chiều dài (cm)
        width: 20, // Chiều rộng (cm)
        height: 20, // Chiều cao (cm)
        insurance_value: 1000000, // Giá trị hàng hóa (VNĐ)
        service_id: 53330, // ID dịch vụ vận chuyển (có thể thay đổi)
    ...
    <div className="flex justify-between border-b py-2">
        ...
        <span>${cart.cartItems.reduce((total, item) => total + item.totalPrice, 0).toFixed(2)}</span>
    </div>
    <div className="flex justify-between border-b py-2">
    ...
    <div className="flex justify-between font-bold text-lg py-2">
        <span>Total Pay:</span>
        <span>${(cart.cartItems.reduce((total, item) => total + item.totalPrice, 0) + shippingFee + 33).toFixed(2)}</span>
    </div>
</div>
                    ...
                    >
                        ...
                        >
                            {({ isSubmitting, setFieldValue }) => (
                                <Form>
                                    <Field as={TextField} name="streetAddress" label="Street Address" fullWidth margin="normal" required />
                                    <ErrorMessage name="streetAddress" component="div" style={{ color: 'red' }} />

                                    <FormControl fullWidth margin="normal" style={{ minWidth: 120 }}>
                                        <InputLabel>Province</InputLabel>
                                        <Field name="stateProvince" as={Select} fullWidth onChange={(e) => handleProvinceChange(e, setFieldValue)} required>
                                        ...
                                        <ErrorMessage name="stateProvince" component="div" style={{ color: 'red' }} />
                                    </FormControl>

                                    <FormControl fullWidth margin="normal" style={{ minWidth: 120 }}>
                                        <InputLabel>District</InputLabel>
                                        <Field name="district" as={Select} fullWidth onChange={(e) => handleDistrictChange(e, setFieldValue)} disabled={!districts.length} required>
                                    ...
                                    <FormControl fullWidth margin="normal" style={{ minWidth: 120 }}>
                                        ...
                                        <Field name="ward" as={Select} fullWidth onChange={(e) => handleWardChange(e, setFieldValue)} disabled={!wards.length} required>
                                            {wards.map((ward) => (
                                    ...
                                    <Field as={TextField} name="postalCode" label="Postal Code" fullWidth margin="normal" required />
